...
def loadData(lmDataBasepath=None, targetSize=64, inputMode=2, trainSplitFactor=1 / 2., fixedNumTrain=0, seqNames=[], zRotInv=[]):
    ...
    if not seqNames:
        seqNames = ("ape", "benchviseblue", "bowl", "cam", "can", "cat", "cup",
...
#         cv2.waitKey()
        ...
        tmplRots = numpy.array(
            [patch.frame.pose.relCamPosZRotInv for patch in tmplSeq.data])
        tmplRots = tmplRots / \
            numpy.tile(numpy.sqrt((tmplRots**2).sum(axis=1)), (3, 1)).T

        # step 1, find closest templates
        rots = numpy.array(
            [patch.frame.pose.relCamPosZRotInv for patch in seq.data])
        rots = rots / numpy.tile(numpy.sqrt((rots**2).sum(axis=1)), (3, 1)).T

        # DEBUG
        print("rots mn/mx x {},{}, y {},{}, z:{},{}".format(numpy.min(rots[:, 0]), numpy.max(rots[
              :, 0]), numpy.min(rots[:, 1]), numpy.max(rots[:, 1]), numpy.min(rots[:, 2]), numpy.max(rots[:, 2])))
        ...
        for j in xrange(numTmpl):
            ...
            if zRotInv[i] == 2:   # half rot invariant
                sim[j, :] = numpy.maximum(
                    sim[j, :], numpy.dot(rots * numpy.array([-1, -1, 1]), tmplRot))
        ...
        for j in xrange(numTmpl):
            ...
            # frames that have j as most similar template
            frIdx, = (maxSimTmplIdx == j).nonzero()
            ...
            if len(frIdx) > 0:
                if fixedNumTrain > 0:
                    ...
                    nTrn = numpy.ceil(
                        len(frIdx) * trainSplitFactor).astype(numpy.int)
                nTst = len(frIdx) - nTrn
        ...
        testSeqs[seqName] = NamedImgSequence(
            seqName, [seq.data[j] for j in testIdx])
...
def createTripleAndPairsDatasetFromImgSeq(tmplSeqs, trainSeqs, inputMode=0, batchSize=None):
    ...
    numTrainPerClass = numpy.array(
        [len(trainSeqs[seqN].data) for seqN in trainSeqs])
    assert numpy.count_nonzero(
        numTrainPerClass - numTrainPerClass[0]) == 0, "cannot deal with unequal training sequence sizes"
    ...
    sim = numpy.zeros(
        (numClasses, numTmplPerClass, numTrainPerClass), dtype=numpy.float)
    ...
    trainRots = numpy.zeros(
        (numClasses, numTrainPerClass, 3), dtype=numpy.float)
    #trainZRI = numpy.zeros((numClasses,numTrainPerClass,),dtype=numpy.uint8)
    for c in xrange(numClasses):

        trots = numpy.array(
            [patch.frame.pose.relCamPosZRotInv for patch in tmplSeqs[seqNames[c]].data])
        tmplRots[c] = trots / \
            numpy.tile(numpy.sqrt((trots**2).sum(axis=1)), (3, 1)).T
        trots = numpy.array(
            [patch.frame.pose.relCamPosZRotInv for patch in trainSeqs[seqNames[c]].data])
        trainRots[c] = trots / \
            numpy.tile(numpy.sqrt((trots**2).sum(axis=1)), (3, 1)).T
        for i in xrange(numTmplPerClass):
            sim[c, i, :] = numpy.dot(trainRots[c], tmplRots[c, i])
            if zRotInv[c] == 2:
                sim[c, i, :] = numpy.maximum(sim[c, i, :], numpy.dot(
                    trainRots[c] * numpy.array([-1, -1, 1]), tmplRots[c, i]))
...
#     cv2.waitKey()
    ...
    #n = 0
    # numTmpls = numpy.zeros(numClasses) # how many tmpls were chosen in this
    # batch
    ...
    imgW = imgSize[1]
    batchData = [
        numpy.zeros((batchSize, nChan, imgH, imgW), dtype=floatX) for nChan in nChans]
    ...
    # index of the training samples and tmpls that landed in the batch
    batchSampIdx = numpy.zeros((batchSize,), dtype=numpy.int32)
    ...
    allTripletIdx = numpy.zeros(
        (maxNumBatches * nTripletsPerBatch, 3), dtype=numpy.int32)
    allPairIdx = numpy.zeros(
        (maxNumBatches * nPairsPerBatch, 2), dtype=numpy.int32)
    allMaxSimTmplIdx = numpy.zeros(
        (maxNumBatches * batchSize,), dtype=numpy.int32)
    # index of the training samples and tmpls that landed in the batch
    allSampIdx = numpy.zeros((maxNumBatches * batchSize,), dtype=numpy.int32)
    allTmplBatchDataStartIdx = numpy.zeros(
        (maxNumBatches, numClasses), dtype=numpy.int32)
    allRots = numpy.zeros((maxNumBatches * batchSize, 3), dtype=numpy.float32)

    mb = (sum([d.nbytes for d in allData]) if isinstance(
        allData, list) else allData.nbytes) / (1024. * 1024.)
    ...
    # keep creating batches until no more data  #  (trainSampIdx <
    # numTrainPerClass)
    while not allTrainSamplesUsed:
        ...
        # idx within batch where the next sample (train and further down
        # template) goes
        k = 0
        while k + totalNumChosenTmpl < batchSize:
            ...
            if currentClass >= numClasses:
                ...
                if trainSampIdx >= numTrainPerClass:
                    print(
                        "no more training samples -> start from beginning, but set a flag so we know this is the last batch")
                    allTrainSamplesUsed = True
                    trainSampIdx = 0
                    # break

            # we need space for the chosen, or at least two templates of each
            # class
            if k + numpy.maximum(totalNumChosenTmpl, 2 * numClasses) >= batchSize:
                # then we're done
                break
...
#         print("---------")
        ...
        for c in range(len(chosenTmpls)):
            ...
            if len(chosenTmpls[c]) < 2:
                ...
                if len(chosenTmpls[c]) == 0:
                    chosenTmpls[c] = numpy.random.choice(
                        numTmplPerClass, 2, replace=False)
                else:
                    x = numpy.random.randint(numTmplPerClass - 1)
                    if x >= chosenTmpls[c]:
                        x += 1
                    chosenTmpls[c] = numpy.concatenate(
                        (chosenTmpls[c], numpy.array([x])))
                totalNumChosenTmpl += len(chosenTmpls[c])
        ...
        while batchNumTrainSamples + totalNumChosenTmpl > batchSize:
            # cut away chosen templates. start with the class that has most
            # templates
            maxCTmpls = 0
            maxCTmplsIdx = 0
            for i in range(len(chosenTmpls)):
...
#             print("{}, {}: {} ({})".format(i,len(chosenTmpls[i]),chosenTmpls[i],type(chosenTmpls[i])))
        ...
        # check that for each class of which we have a label in the batch there
        # is at least one template
        for i in range(batchNumTrainSamples):
            if len(chosenTmpls[batchLabels[i]]) <= 0:
                raise ValueError(
                    "no template for class {}".format(batchLabels[i]))
            if len(chosenTmpls[batchLabels[i]]) < 2:
                raise ValueError(
                    "only one template for class {}".format(batchLabels[i]))

        #print("add templates to batch")
        tmplBatchDataStartIdx = numpy.zeros(numClasses, dtype=numpy.int32)
        # for every chosen tmpl the index where it ends up in the batch (-1 for
        # all others)
        tmplBatchDataIdx = - \
            numpy.ones((numClasses, numTmplPerClass), dtype=numpy.int32)
        for c in xrange(numClasses):
            ...
            if len(chosenTmpls[c]) > 0:
                ...
                chosenTmpls[c] = numpy.array(
                    list(chosenTmpls[c])).astype(numpy.int32)
                tmplBatchDataIdx[c, chosenTmpls[c]] = numpy.arange(
                    len(chosenTmpls[c])) + k
                for l in xrange(len(chosenTmpls[c])):
                    ...
                    if k >= batchSize:
                        for cx in xrange(numClasses):
                            print("{}: {}".format(cx, len(chosenTmpls[cx])))
                        raise ValueError("Cannot fit all chosen tmpls in batch. This shouldn't happen anymore ({},{},{},{},{},{})".format(
                            k, batchSize, l, c, batchNumTrainSamples, totalNumChosenTmpl))

                    cTmplIdx = chosenTmpls[c][l]

                    imgD = dataArraysFromPatch(
                        tmplSeq.data[cTmplIdx], inputMode)
        ...
        batchTripletIdx[0:batchNumTrainSamples * 3,
                        0] = numpy.tile(numpy.arange(batchNumTrainSamples), (1, 3))

        # second is the maxSimTmpl
        # it was batchMaxSimTmplIdx, but where did it end up in the batchData?
        maxSimTmplDataIdx = numpy.zeros(
            (batchNumTrainSamples,), dtype=numpy.int32)
        for i in xrange(batchNumTrainSamples):
            c = batchLabels[i]
            maxSimTmplDataIdx[i] = tmplBatchDataIdx[c][batchMaxSimTmplIdx[i]]
            # we couldn't fit its closest into the batch, then what? -> take
            # the best of those that are there
            if maxSimTmplDataIdx[i] < 0:
                ...
                maxSimTmplDataIdx[i] = numpy.argmax(
                    sim[c, availableSameClassTmpls, batchSampIdx[i]]) + tmplBatchDataStartIdx[c]
                #print(" -> choose {}".format(maxSimTmplDataIdx[i]))

        batchTripletIdx[
            0:batchNumTrainSamples * 3, 1] = numpy.tile(maxSimTmplDataIdx, (1, 3))
        ...
        for i in xrange(batchNumTrainSamples):
            ...
            # the chosen ones from the same class
            otherIdx = numpy.arange(
                len(chosenTmpls[c])) + tmplBatchDataStartIdx[c]
            # remove those that have the same similarity to train as the
            # closest tmpl
            ...
            if len(otherIdx) > 0:
                ...
                # that's not good, but for now lets just take one of another
                # class
                otherClass = zeroSaveRandInt(numClasses - 1)
                if otherClass >= c:
                    otherClass += 1
                batchTripletIdx[i, 2] = zeroSaveRandInt(
                    len(chosenTmpls[otherClass])) + tmplBatchDataStartIdx[otherClass]
        ...
        for i in xrange(batchNumTrainSamples):
            ...
            if len(otherIdx) > 0:
                ...
                if otherClass >= c:
                    otherClass += 1
                batchTripletIdx[tidx, 2] = zeroSaveRandInt(
                    len(chosenTmpls[otherClass])) + tmplBatchDataStartIdx[otherClass]
        ...
        for i in xrange(batchNumTrainSamples * 3, nTripletsPerBatch):
            ...
            batchTripletIdx[i, 1] = zeroSaveRandInt(
                len(chosenTmpls[c])) + tmplBatchDataStartIdx[c]  # random tmpl of same class
            c2 = zeroSaveRandInt(numClasses)
            if c2 == c:
                batchTripletIdx[i, 2] = zeroSaveRandInt(
                    len(chosenTmpls[c]) - 1) + tmplBatchDataStartIdx[c]
                if (batchTripletIdx[i, 2] >= batchTripletIdx[i, 1]) and (len(chosenTmpls[c]) > 1):
                    batchTripletIdx[i, 2] += 1
                # now check if the tmpl in batchTripletIdx[i,2] is actually
                # more similar than batchTripletIdx[i,1]
                if (batchTripletIdx[i, 1] < 0) or (batchTripletIdx[i, 1] >= len(batchLabels)):
                    print("------- FAIL, triplet idx[1] wrong")
                    print(
                        "{}, {}".format(batchTripletIdx[i, 1], len(batchLabels)))
                    print(batchTripletIdx)
                    print(batchLabels)
                    print(chosenTmpls)
                    print(tmplBatchDataStartIdx)
                    assert False
                assert batchLabels[batchTripletIdx[
                    i, 1]] == c, "batchlabels {}, batchTripletIdx {}".format(batchLabels, batchTripletIdx)
                ...
                assert batchLabels[batchTripletIdx[i, 2]] == c2, "{}, {}, {}, {}".format(
                    batchLabels[batchTripletIdx[i, 2]], c2, c, len(chosenTmpls[c]))
                s1 = batchSampIdx[batchTripletIdx[i, 1]]
                s2 = batchSampIdx[batchTripletIdx[i, 2]]
                # then we have to swap them
                if sim[c, s2, trIdx] > sim[c, s1, trIdx]:
                    h = batchTripletIdx[i, 2]
                    batchTripletIdx[i, 2] = batchTripletIdx[i, 1]
                    batchTripletIdx[i, 1] = h
                # else:
                # if sim[c,s2,trIdx] == sim[c,s1,trIdx]: # then we have to find
                # another one
            else:
                batchTripletIdx[i, 2] = zeroSaveRandInt(
                    len(chosenTmpls[c2])) + tmplBatchDataStartIdx[c2]
...
#         print("-------------------------------------------")
        ...
        allLabels[numBatches * batchSize:
                  (numBatches + 1) * batchSize] = batchLabels
        # ** + numBatches*batchSize # (absolute idx, absolutely not clever)
        allTripletIdx[numBatches * nTripletsPerBatch:
                      (numBatches + 1) * nTripletsPerBatch] = batchTripletIdx
        # ** + numBatches*batchSize # (absolute idx, absolutely not clever)
        allPairIdx[numBatches * nPairsPerBatch:
                   (numBatches + 1) * nPairsPerBatch] = batchPairIdx
        allMaxSimTmplIdx[
            numBatches * batchSize:(numBatches + 1) * batchSize] = batchMaxSimTmplIdx
        allSampIdx[
            numBatches * batchSize:(numBatches + 1) * batchSize] = batchSampIdx
        allRots[numBatches * batchSize:
                (numBatches + 1) * batchSize] = batchRots
        # ** + numBatches*batchSize  # (absolute idx, absolutely not clever)
        allTmplBatchDataStartIdx[numBatches] = tmplBatchDataStartIdx
        numBatches += 1
    ...
    allPairLabels = numpy.ones(
        (numBatches * nPairsPerBatch,), dtype=numpy.int32)

    #trainRots = trainRots.reshape((numTrainPerClass*numClasses,3))
    #tmplRots = tmplRots.reshape((numTmplPerClass*numClasses,3))
    sampleInfo = {'maxSimTmplIdx': allMaxSimTmplIdx, 'sampIdx': allSampIdx, 'tmplBatchDataStartIdx': allTmplBatchDataStartIdx,
                  'rots': allRots, 'tmplRots': tmplRots, 'trainRots': trainRots, 'nTrainPerSeq': numTrainPerClass, 'zRotInv': zRotInv}

    data_set = BatchedLinemodTrainDataset(allData, y=allLabels, sampleInfo=sampleInfo, pairIdx=allPairIdx, pairLabels=allPairLabels,
                                          tripletIdx=allTripletIdx, nPairsPerBatch=nPairsPerBatch, nTripletsPerBatch=nTripletsPerBatch, batchSize=batchSize)
...
def bootstrapTriplets(descrNetTrainer, train_set):
    ...
    newTripletIdx = numpy.zeros(
        (numBatches * newNTripletsPerBatch, 3), dtype=numpy.int32)
    ...
    for nBatch in xrange(numBatches):
        # add old triplets
        newTripletIdx[nBatch * newNTripletsPerBatch:nBatch * newNTripletsPerBatch +
                      oldNTripletsPerBatch] = train_set.tripletIdx[nBatch * oldNTripletsPerBatch:(nBatch + 1) * oldNTripletsPerBatch]

        # calc distances
        # prefix b = batch -> descriptors of the current batch
        bDescrs = descrs[nBatch * batchSize:(nBatch + 1) * batchSize]
        ...
        # assuming there was an tripletIdx in the train_set where for each
        # sample there was a triplet with the target tmpl as the "close" sample
        targetTmplIdx = train_set.tripletIdx[
            nBatch * oldNTripletsPerBatch:nBatch * oldNTripletsPerBatch + bNumTrain, 1] - bNumTrain
...
#             k += 1
        ...
        k = nBatch * newNTripletsPerBatch + oldNTripletsPerBatch
        i = 0
        # take closest wrong tmpl (and increment this every time we are through
        # with one loop over all samples)
        tmplListIdx = 0
        # as long as there are empty slots for new triplets
        while k < batchTripletIdxEnd:
            # prefix s = sample .... sLabel is the sample's label
            sLabel = bLabels[i]

            #--- closest from other class
            minDstOtherClassTmplIdx = numpy.argsort(
                dst[i, bOtherClassTmplIdx[sLabel]])
            #print("minDstOtherClassTmplIdx.shape {}".format(minDstOtherClassTmplIdx.shape))
            minDstOtherClassTmplIdx = minDstOtherClassTmplIdx[
                numpy.minimum(tmplListIdx, len(minDstOtherClassTmplIdx) - 1)]
            # fuck this is strange. but it's correct  (right?)
            minDstOtherClassTmplIdx = bOtherClassTmplIdx[
                sLabel][minDstOtherClassTmplIdx]
            ...
            # set target tmpl to have a high distance, such that it wont be
            ...
            # orig sample indizes of the templates of the same class
            sameClassTmplSampIdx = sampIdx[
                nBatch * batchSize + bNumTrain + bSameClassTmplIdx[sLabel]]
            trRot = trainRots[sLabel, trSampIdx]
            scTmplRots = tmplRots[sLabel, sameClassTmplSampIdx]
            # similarities of poses of training sample i and
            sims = numpy.dot(scTmplRots, trRot.T)
            if zRotInv[sLabel] == 2:
                sims = numpy.maximum(
                    sims, numpy.dot(scTmplRots, (trRot * numpy.array([-1, -1, 1])).T))
            ...
            if numEqualSameClassTmpls < len(bSameClassTmplIdx[sLabel]):
                ...
                minDstSameClassTmplIdx = numpy.argsort(
                    dst[i, bSameClassTmplIdx[sLabel]])
                minDstSameClassTmplIdx = minDstSameClassTmplIdx[
                    numpy.minimum(tmplListIdx, len(minDstSameClassTmplIdx) - 1)]
                minDstSameClassTmplIdx = bSameClassTmplIdx[
                    sLabel][minDstSameClassTmplIdx]
            ...
            if i >= bNumTrain:  # loop over training samples
                i = 0
                # when all training samples where considered, take the next
                # best (second/third/... closest tmpl) as "far" tmpl
                tmplListIdx += 1
...
def checkDataset(train_set):
    ...
    assert nBatches * \
        batchSize == numSamples, "number of samples {} not divisible by batchSize {}".format(
    ...
    # number of the sample in original per-class sequence
    sampIdx = si['sampIdx']
    ...
    for nBatch in xrange(nBatches):
        for i in xrange(nTripletsPerBatch):
            ...
            if idx[0] >= tmplStartIdx[nBatch, 0]:
                print("ERROR: first index must be train sample but {} >= {}".format(
                    numpy.max(idx[0]), tmplStartIdx[nBatch, 0]))
            # check if idx1,idx2 are in the template sample area
            if idx[1] < tmplStartIdx[nBatch, 0]:
                print("ERROR: second index must be template sample but {} < {}".format(
                    numpy.max(idx[1]), tmplStartIdx[nBatch, 0]))
            if idx[2] < tmplStartIdx[nBatch, 0]:
                print("ERROR: third index must be template sample but {} < {}".format(
                    numpy.max(idx[2]), tmplStartIdx[nBatch, 0]))

            idx = numpy.copy(idx)
            # it is now a within batch idx. so to index into the whole dataset
            # add offset
            idx = idx + nBatch * batchSize
            ...
            if l0 != l1:
                ...
                # and if yes, whether the rotation of the second is bigger than
                # the first
                if (l0 == l2):
                    ...
                    if zRotInv[l0] == 2:
                        sim1 = numpy.maximum(
                            sim1, numpy.dot(rot0 * numpy.array([-1, -1, 1]), rot1))
                        sim2 = numpy.maximum(
                            sim2, numpy.dot(rot0 * numpy.array([-1, -1, 1]), rot2))
                    if sim1 < sim2:
                        print("ERROR: s2 is more similar to s0 than s1 !!")
                        print(
                            "   idx[0] = {}, [1] = {}, [2] = {}".format(idx[0], idx[1], idx[2]))
                        print("   sampIdx[0] = {}, [1] = {}, [2] = {}".format(
                            sampIdx[idx[0]], sampIdx[idx[1]], sampIdx[idx[2]]))
                        print(
                            "   rot0[0] = {}, 1 = {}, 2 = {}".format(rot0, rot1, rot2))
...
def printDatasetInfo(data_set):
    ...
    # sum([x.nbytes for x in data_set.x]) / nBatches
    batchSizeBytes = data_set.dataSizeNBytes
...
def makeTrainsetSizeAMultipleOfValsetSize(train_set, val_set):
    ...
    print(
        "--- valset   nsamp {},  batchsize {}".format(nValSamples, val_set.batchSize))
    print(
        "--- trainset nsamp {},  batchsize {}".format(nTrainSamples, train_set.batchSize))
    if numpy.mod(nTrainBatches, nValBatches) != 0:
        if nTrainBatches > nValBatches:
            n = numpy.ceil(nTrainBatches / float(nValBatches))
            # d1 = number of batches to add to train
            d1 = numpy.floor(nValBatches * n - nTrainBatches).astype(int)
            d2 = 0
            # d2 = number of add to val to make val size a divisor of train
            # size
            while numpy.mod(nTrainBatches, nValBatches + d2) > 0:
                d2 += 1
            print("d1 {}, d2 {}".format(d1, d2))
            if d1 < d2:
                # add d1 random minibatches to the end of train
                print("--> stacking up train_set by {}, to {} (val {})".format(d1,
                                                                               nTrainBatches + d1, nValBatches))
                idx = numpy.random.permutation(nTrainBatches)
                idx = idx[:d1]
                train_set.extendSetBySubset(idx)
            else:
                # add d2 random minibatches to the end of val
                print("--> stacking up val_set by {}, to {} (train {})".format(d2,
                                                                               nValBatches + d2, nTrainBatches))
                idx = numpy.random.permutation(nValBatches)
                idx = idx[:d2]
                val_set.extendSetBySubset(idx)
        else:
            n = numpy.ceil(nValBatches / float(nTrainBatches))
            # d1 = number of batches to add to val
            d1 = numpy.floor(nTrainBatches * n - nValBatches).astype(int)
            d2 = 0
            # d2 = number of add to train to make val size a divisor of val
            # size
            while numpy.mod(nValBatches, nTrainBatches + d2) > 0:
                d2 += 1
            print("reverse, d1 {}, d2 {}".format(d1, d2))
            if d1 < d2:
                # add d1 random minibatches to the end of val
                print("--> stacking up val_set by {}, to {} (train {})".format(d1,
                                                                               nValBatches + d1, nTrainBatches))
                idx = numpy.random.permutation(nValBatches)
                idx = idx[:d1]
                val_set.extendSetBySubset(idx)
            else:
                # add d2 random minibatches to the end of train
                print("--> stacking up train_set by {}, to {} (val {})".format(d2,
                                                                               nTrainBatches + d2, nValBatches))
                idx = numpy.random.permutation(nTrainBatches)
                idx = idx[:d2]
                train_set.extendSetBySubset(idx)
...
def findNiceMacroBatchSize(macroBatchSize, maxBytes, batchSizeBytes, datasetsNBatches=[]):
    ...
    print(
        "Adjusting MacroBatch size such that a multiple of it fits into the number of batches of all datasets")
    ...
    # how many batches fit into memory and are dividers of the total number of
    # batches
    maxMacroBatchSize = numpy.floor(
        maxBytes / batchSizeBytes).astype(numpy.int)
    if len(datasetsNBatches) > 0:
        ...
        if macroBatchSize > maxMacroBatchSize:
            ...
            # okay. let's not try to be smart here. just go down from this
            # maxMacroBatchSize until we have a common divisor
...
def prepareData(cfg, rng):
    ...
    # split this number of training from the test sequences
    fixedNumTrain = readCfgIntParam(
        cfg, 'trainset', 'fixedNumTrain', default=1)

    numNoisyTrainCopies = readCfgIntParam(
        cfg, 'trainset', 'numNoisyTrainCopies', default=7)
    trainSplitFactor = readCfgFloatParam(
...
#     cv2.waitKey(0)
    ...
    if loadTrainSetFromPkl and os.path.isfile(trainSetPklFileName):
        ...
        if (not loadImgsFromPkl) or (not os.path.isfile(imgsPklFileName)):
            ...
            tmplSeqs, trainSeqs, testSeqs = loadData(
...
#                 placeholder_patch.frame.pose.relCamOrient))
            ...
            for seqN in trainSeqs:
                if len(trainSeqs[seqN].data) < numTrainPerClass:
                    idx = numpy.random.choice(
                        len(trainSeqs[seqN].data), numTrainPerClass - len(trainSeqs[seqN].data), replace=False)
                    for i in idx:
        ...
        for i in range(numNoisyTrainCopies):
            ...
            addNoiseToTrainSeqs(trSeqs, inputMode, rng, addSoftNoise=True,
                                softNoiseRange=0.01, addHardNoise=False, addBgNoise=True)

            # create dataset from img sequences
            if train_set is None:
                train_set = createTripleAndPairsDatasetFromImgSeq(
                    tmplSeqs, trSeqs, inputMode=inputMode, batchSize=batchSize)
                val_set = createTripleAndPairsDatasetFromImgSeq(
                    tmplSeqs, trSeqs, inputMode=inputMode, batchSize=batchSize)
            else:
                ts = createTripleAndPairsDatasetFromImgSeq(
                    tmplSeqs, trSeqs, inputMode=inputMode, batchSize=batchSize)
                train_set.append(ts)
    ...
    maxBytes = eval(
        readCfgParam(cfg, 'trainset', 'maxBytes', default=50 * 1024 * 1024))  # 1 GB
    batchSizeBytes = train_set.dataSizeNBytes / nTrainBatches

    if macroBatchSize < 0:
        macroBatchSize = findNiceMacroBatchSize(
            macroBatchSize, maxBytes, batchSizeBytes, [nTrainBatches, nValBatches])
    #---------------------------------------------------------------
...
def createNetwork(rng, cfg, train_set, graphOutFile=None):
    ...
    if inputMode == 0:
        ...
        inputDim = [[train_set.batchSize, 3, imgSizeH, imgSizeW],
                    [train_set.batchSize, 1, imgSizeH, imgSizeW]]
    else:
    ...
    assert outputDescrLen == descrNetParams.outputDim[
        1], "outputDescrLen set in the network config does not match what the defined net calculates"
...
def createTrainer(rng, cfg, descrNet, train_set, val_set, dataManager, resPath):
    ...
    descrNetTrainerParams.use_pairs = (
        readCfgIntParam(cfg, 'train', 'use_pairs', 1) == 1) and pairsAvailable
    descrNetTrainerParams.learning_rate = readCfgFloatParam(
        cfg, 'train', 'learning_rate', default=0.01)
    descrNetTrainerParams.learning_rate_schedule_iter = readCfgIntParam(
        cfg, 'train', 'learning_rate_schedule_iter', default=nTrainBatches * 100)  # decrease after every n minibaches (every 100 epochs)
    descrNetTrainerParams.learning_rate_schedule_factor = readCfgFloatParam(
        cfg, 'train', 'learning_rate_schedule_factor', default=0.9)  # decrease by 0.9
    descrNetTrainerParams.save_traindescr_interval = readCfgIntParam(
        cfg, 'train', 'save_traindescr_interval', default=0)  # decrease after every n minibaches (every 100 epochs)
    descrNetTrainerParams.save_traindescr_filebase = readCfgParam(
        cfg, 'train', 'save_traindescr_filebase', default='')  # decrease after every n minibaches (every 100 epochs)
    descrNetTrainerParams.momentum = readCfgFloatParam(
        cfg, 'train', 'momentum', default=0.9)
    # descrNetTrainerParams.weightreg_factor = 1e-3#1e-7
    descrNetTrainerParams.weightreg_factor = readCfgFloatParam(
        cfg, 'train', 'weightreg_factor', default=1.0)  # 1e-4, 1e-2

    descrNetTrainerParams.tripletCostType = readCfgIntParam(
        cfg, 'train', 'tripletCostType', default=6)  #
    descrNetTrainerParams.tripletThresh = readCfgFloatParam(
        cfg, 'train', 'tripletThresh', default=0.01)
    #descrNetTrainerParams.tripletThresh = 0.001

    # descrNetTrainerParams.nClasses = numClasses # only needed if use_labels
    # = True
    ...
    descrNetTrainer.setDataAndCompileFunctions(
        train_set, val_set, dataManager, compileDebugFcts=True)
...
def linemod_train_realandsynth_main(cfg):
    ...
    resPath = os.path.abspath(os.path.dirname(
        os.path.abspath(__file__)) + "/../data/results/linemod_realandsynth/") + "/"
    ...
    ##########################################################################
    # CREATE TRAINER
    descrNetTrainer = createTrainer(
        rng, cfg, descrNet, train_set, val_set, dataManager, resPath)
    assert isinstance(
        descrNetTrainer, NetworkTrainer), "ERROR: creating trainer failed"
    ...
    calcAndVis3DDescriptorsWithRotWithTrainer(
        descrNetTrainer, vis_eval_set, resPath + startTimePidStr + "_descr_init.png")
    visNetworkFiltersMontage(
        descrNet, resPath + startTimePidStr + "_filters_init.png")
    if showPlots:
    ...
    numBootstrappingRounds = readCfgIntParam(
        cfg, 'train', 'numBootstrappingRounds', default=3)
    nBootstrappingEpochs = readCfgIntParam(
        cfg, 'train', 'nBootstrappingEpochs', default=500)
    nFinetuneingEpochs = readCfgIntParam(
        cfg, 'train', 'nFinetuneingEpochs', default=500)
    ...
    saveTrainResultsPickle(
        resPath, fn, descrNet, trainProgressData, cfg, descrNetTrainer.cfgParams)
    ...
    #  DEBUG VISUALIZATIONS
    calcAndVis3DDescriptorsWithRotWithTrainer(
        descrNetTrainer, vis_eval_set, resPath + startTimePidStr + "_descr_trained0.png")
    visNetworkFiltersMontage(
        descrNet, resPath + startTimePidStr + "_filters.png")
    ...
    for i in xrange(numBootstrappingRounds):
        ...
        train_start_time = time.clock()
        train_res = descrNetTrainer.train(
            n_epochs=nBootstrappingEpochs, iterOffset=nIter)
        train_end_time = time.clock()
        ...
        calcAndVis3DDescriptorsWithRotWithTrainer(
            descrNetTrainer, vis_eval_set, resPath + startTimePidStr + "_descr_trained_b{}.png".format(i + 1))
        visNetworkFiltersMontage(
            descrNet, resPath + startTimePidStr + "_filters_b{}.png".format(i + 1))
    ...
    if nFinetuneingEpochs > 0:
        ...
        train_start_time = time.clock()
        train_res = descrNetTrainer.train(
            n_epochs=nFinetuneingEpochs, iterOffset=nIter)
        train_end_time = time.clock()
        print ('>> Training took %.1fs' %
               ((train_end_time - train_start_time)))
        ...
        calcAndVis3DDescriptorsWithRotWithTrainer(
            descrNetTrainer, vis_eval_set, resPath + startTimePidStr + "_descr_trained_final.png")
        visNetworkFiltersMontage(
            descrNet, resPath + startTimePidStr + "_filters_final.png")
    ...
    # TODO write config/params as cfg/ini/xml - file as well so we dont depend
    # on the pickle (if the pickle cant be loaded anymore and we still what to
    # see the config that led to the results)

    ##################
    end_time = time.clock()
    print ('The code for file ' + os.path.split(__file__)
           [1] + ' ran for %.1fs' % ((end_time - start_time)))
    print("done")

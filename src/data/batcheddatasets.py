...
class BatchedImgSeqDataset(BatchedDataset):
    ...
    def __init__(self, x=None, y=None, yr=None, sampleInfo=None, batchSize=None):
        super(BatchedImgSeqDataset, self).__init__(
            x, y, yr, sampleInfo, batchSize)
    ...
    def initFromImgSeqs(self, imgSeqs, inputMode, batchSize):
        ...
        if isinstance(imgSeqs, dict):
            # fucking unsorted dicts, hate you
            seqNames = sorted(imgSeqs.keys())
            imgSeqsList = []
        ...
        if inputMode == 3:
            ...
            print("nChan {}, patchH {}, patchW {}".format(
                nChans, patchH, patchW))
        else:
            nChan, patchH, patchW = imgD0[0].shape
            nChans = [nChan]
            print(
                "nChan, patchH, patchW {},{},{}".format(nChan, patchH, patchW))

        #nChan = getNChanFromInputMode(inputMode)
        # integer ceil
        numBatches = (totalNumImgs + batchSize - 1) // batchSize
        allData = [numpy.zeros(
            (numBatches * batchSize, nChan, patchH, patchW), dtype=floatX) for nChan in nChans]
        ...
        for i in xrange(len(imgSeqs)):
            for j in xrange(len(imgSeqs[i].data)):
                ...
                if zRotInv[i] < 0:
                    ...
                    raise ValueError("zRotInv inconsistent, class {}: {} != {}".format(
                        i, zRotInv[i], patch.frame.pose.zRotInv))
                allSampIdx[k] = j
                k += 1
...
class BatchedLinemodTrainDataset(BatchedTrainDataset):
    ...
    def append(self, other):
        ...
        self.sampleInfo['maxSimTmplIdx'] = numpy.concatenate(
            (self.sampleInfo['maxSimTmplIdx'], other.sampleInfo['maxSimTmplIdx']))
        self.sampleInfo['sampIdx'] = numpy.concatenate(
            (self.sampleInfo['sampIdx'], other.sampleInfo['sampIdx']))
        ...
        self.sampleInfo['tmplBatchDataStartIdx'] = numpy.concatenate(
            (self.sampleInfo['tmplBatchDataStartIdx'], other.sampleInfo['tmplBatchDataStartIdx']))
        self.sampleInfo['rots'] = numpy.concatenate(
            (self.sampleInfo['rots'], other.sampleInfo['rots']))

        # not necessary. instead we should assert that they are the same,
        # because the sampIdx is used to index them and its not adjusted
        ...
        assert numpy.allclose(
            self.sampleInfo['trainRots'], other.sampleInfo['trainRots'])
        assert numpy.allclose(
            self.sampleInfo['zRotInv'], other.sampleInfo['zRotInv'])
    ...
    def extendSetBySubset(self, batchIdx):
        ...
        sampIdxInBatch = numpy.tile(
            numpy.arange(self.batchSize), len(batchIdx))
        batchOffset = numpy.repeat(batchIdx, self.batchSize) * self.batchSize
        sampIdx = batchOffset + sampIdxInBatch

        self.sampleInfo['maxSimTmplIdx'] = numpy.concatenate(
            (self.sampleInfo['maxSimTmplIdx'], self.sampleInfo['maxSimTmplIdx'][sampIdx]))
        self.sampleInfo['sampIdx'] = numpy.concatenate(
            (self.sampleInfo['sampIdx'], self.sampleInfo['sampIdx'][sampIdx]))
        self.sampleInfo['tmplBatchDataStartIdx'] = numpy.concatenate(
            (self.sampleInfo['tmplBatchDataStartIdx'], self.sampleInfo['tmplBatchDataStartIdx'][batchIdx]))
        self.sampleInfo['rots'] = numpy.concatenate(
            (self.sampleInfo['rots'], self.sampleInfo['rots'][sampIdx]))


class LinemodBatchedTestData(BatchedImgSeqDataset):

    def __init__(self, x=None, y=None, yr=None, sampleInfo=None, batchSize=None):
        super(LinemodBatchedTestData, self).__init__(
            x, y, yr, sampleInfo, batchSize)
    ...
    def addFromCroppedGTpatches(self, lmDataset, inputMode=0, targetSize=64, cropSize=20.0, batchSize=None):
        ...
        assert (self.batchSize is None) or (
            self.batchSize == batchSize), "self.batchSize {} != new batchSize {}".format(self.batchSize, batchSize)
        ...
        if self.x is None:
            ...
            print("#-# add")
            self.x = [numpy.concatenate((x, d)) for x, d in izip(self.x, allData)] if (
                inputMode == 3) else numpy.concatenate((self.x, allData[0]))
...
#         cv2.imshow("test",im)
        ...
        if inputMode != 1:
            ...
            if fixHolesMode == 0:
                ...
                # dpt =
                # cv2.inpaint(dpt,inpaintMask,inpaintRadius=3,flags=cv2.INPAINT_TELEA)
                # # can only be used on uint8
...
def dataArraysFromPatch(patch, inputMode):
    ...
    if inputMode == 0:
        ...
        if patch.dpt is not None:
            imgD = [
                patch.dpt.reshape((1, patch.dpt.shape[0], patch.dpt.shape[1]))]
        else:
            print(
                "WARNING: batcheddatasets.dataArraysFromPatch:  patch.dpt is empty")
        ...
        if patch.img is not None:
            ...
            print(
                "WARNING: batcheddatasets.dataArraysFromPatch:  patch.img is empty")
        ...
        if (patch.img is not None) and (patch.dpt is not None):
            ...
            print(
                "WARNING: batcheddatasets.dataArraysFromPatch:  patch.img or patch.dpt is empty")
        ...
        if (patch.img is not None) and (patch.dpt is not None):
            ...
            imgD = numpy.rollaxis(imgD, 2)
            imgD = [
                imgD[0:3], imgD[3].reshape((1, imgD.shape[1], imgD.shape[2]))]
        else:
            print("WARNING: batcheddatasets.dataArraysFromPatch:  patch.{} is empty".format(
                "img" if patch.img is None else "dpt"))

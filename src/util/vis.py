...
def showDistanceMatrix(dst):
    ...
    imgD = ((dst - numpy.min(dst)) * 255.0 /
            (numpy.max(dst) - numpy.min(dst))).astype(numpy.uint8)
...
def visualizeFiltersEvolution(wvals, fileName=None):
    ...
    for j in range(len(idx)):
        ...
        for i in range(N):
            ...
            imgD = ((imgD - numpy.min(imgD)) * 255.0 /
                    (numpy.max(imgD) - numpy.min(imgD))).astype(numpy.uint8)
            if len(imgD.shape) > 2:
                img = Image.fromarray(imgD, mode='RGB')
            else:
                # img = Image.fromarray(imgD) #,mode='L')  # this is color
                # coded
                imgD = imgD.reshape(
                    (imgD.shape[0], imgD.shape[1], 1)).repeat(3, axis=2)
...
#     print("numRows = {}".format(numRows))
    ...
    wAll = numpy.zeros(
        (numRows * rowHeight, numCols * colWidth), dtype=numpy.float32)
    wFrame = numpy.zeros(
        (numRows * rowHeight, numCols * colWidth), dtype=numpy.float32)
    for i in xrange(numFilters):
        ...
        filterPatch = wvals[i]
        if nChan > 1:
            filterPatch = numpy.swapaxes(filterPatch, 0, 1).reshape(
                fh, fw * nChan)  # all channels next to each other

        wAll[rowIdx * rowHeight:rowIdx * rowHeight + targetFH,
             colIdx * colWidth:colIdx * colWidth + targetFW] = filterPatch
        wFrame[rowIdx * rowHeight:rowIdx * rowHeight + targetFH, colIdx *
               colWidth:colIdx * colWidth + targetFW] = numpy.ones(filterPatch.shape)
...
def visNetworkFiltersMontage(network, fileName=None):
    ...
    for l in network.layers:
        if isinstance(l, ConvLayer) or isinstance(l, ConvPoolLayer):
            ...
            visFilterMontageOfLayer(
                wvals, fileName=layerFN, windowName=windowName)
...
def visFilters(wvals, Wout, numRows=None, fileName=None):
    ...
    for i in range(numFilters):
        ...
        if nChan > 1:
            ...
            if nChan == 4:
                imgD = numpy.hstack((imgD[:, :, :3], numpy.tile(
                    imgD[:, :, 3].reshape(imgD.shape[0], imgD.shape[1], 1), (1, 1, 3))))
        else:
...
def visualizeNetworkResponse(descrNet, imgStack):
    ...
    if imgStack.ndim == 3:  # only one image. chan x h x w
        imgStack = imgStack.reshape(
            (1, imgStack.shape[0], imgStack.shape[1], imgStack.shape[2]))

    numImgs = imgStack.shape[0]

    outputs = [
        layer.output for layer in descrNet.layers if isinstance(layer, ConvLayer)]
    f = theano.function(
        inputs=[], outputs=outputs, givens={descrNet.inputVar: imgStack})
    layerOutputs = f()
...
#        cv2.waitKey(0)
        ...
        for j in range(len(descrNet.layers)):
            if isinstance(descrNet.layers[j], ConvLayer):
                ...
                respMaps = (respMaps - numpy.min(respMaps)) / \
                    (numpy.max(respMaps) - numpy.min(respMaps))
                fig = plt.figure()
                ax = fig.add_subplot(111)
                ax.axes.set_frame_on(False)
                ax.grid(False)
                # @UndefinedVariable
                ax.matshow(respMaps, cmap=plt.cm.gray)
...
def montageLinemodTrainSamplesAndTmpls(train_set, outFileName=None):
    ...
    for i in xrange(min(nImgs, nCols * nRows)):
        ...
        # skip tmpls
        batchOffset = (
            numpy.floor(i / train_set.batchSize) * train_set.batchSize).astype(numpy.int)
        ...
        print("{}: {},{}".format(i, train_set.sampleInfo['rots'][
              trainIdx], train_set.sampleInfo['rots'][tmplIdx]))
...
def vis3DDescriptors2(descrs, labels, fileName=None):
    ...
    for i in range(3):
        idx = labels == i
        ax.scatter3D(descrs[idx, 0], descrs[idx, 1], descrs[
                     idx, 2], c=colors[i], marker=markers[i])
...
def calcAndVis3DDescriptorsWithRotWithTrainer(trainer, test_data, fileName=None):
    ...
    print("numSamples {}, batch_size {}".format(
        test_data.numSamples, test_data.batchSize))
    ...
    print("test_data.x.shape {}, num valid {}".format(
        test_data.numSamples, numpy.sum(validIdx)))
    vis3DDescriptorsWithRot(res[validIdx], test_data.y[
                            validIdx], test_data.sampleInfo['rots'][validIdx], fileName)
...
def calcAndVis3DDescriptorsWithRot(network, test_data, dataManager=None, fileName=None):
    ...
    res = network.computeDescriptors(
        test_data, dataManager, batch_size=test_data.batchSize)
    # throw away results for invalid samples (that were added to fill up
    # minibatches)
    validIdx = test_data.y >= 0
    print("test_data.x.shape {}, num valid {}".format(
...
#         cv2.line(im,tuple(rectPt4),tuple(rectPt1),color=color)
    ...
    for coord, color in zip(coords, colors):
        pt1 = numpy.round(lcam.worldToScreen(numpy.dot(mat, worldPt1 + numpy.array(
            [rectSize2 * coord[0][0], rectSize2 * coord[0][1], rectSize2 * coord[0][2], 0])))).astype(int)
        pt2 = numpy.round(lcam.worldToScreen(numpy.dot(mat, worldPt1 + numpy.array(
            [rectSize2 * coord[1][0], rectSize2 * coord[1][1], rectSize2 * coord[1][2], 0])))).astype(int)
...
def visBatchedDatasetCamPoses(data_set):
    ...
    uLab = numpy.setdiff1d(
        numpy.unique(data_set.y), numpy.array(-1), assume_unique=True)
    ...
    for i in xrange(numClasses):
        l = uLab[i]
        ax = fig.add_subplot(
            nRows, nCols, i + 1, projection='3d', aspect='equal')
        rl = r[data_set.y == l, :]
        ...
        ax.plot(numpy.array([cx - d, cx - d, cx - d, cx - d, cx + d, cx + d, cx + d, cx + d]),
                numpy.array(
                    [cy - d, cy - d, cy + d, cy + d, cy - d, cy - d, cy + d, cy + d]),
...
def visBatchedTrainDatasetCamPoses(train_set):
    ...
    uLab = numpy.setdiff1d(
        numpy.unique(train_set.y), numpy.array(-1), assume_unique=True)
    numClasses = len(uLab)
    nRows = numpy.round(numpy.sqrt(numClasses / 1.4)).astype(numpy.int)
    nCols = int(numpy.ceil(float(numClasses) / float(nRows)))
    ...
    batchOffsets = (
        numpy.arange(nTriplets) / train_set.nTripletsPerBatch) * batchSize
    trIdx = train_set.tripletIdx + \
        numpy.tile(batchOffsets.reshape((nTriplets, 1)), (1, 3))
    print("labels: {}".format(uLab))
    fig = plt.figure()
    for i in xrange(numClasses):
        l = uLab[i]
        ...
        ax.plot(rotTmpl[:, 0] * 1.2, rotTmpl[:, 1]
                * 1.2, rotTmpl[:, 2] * 1.2, '.')
        ax.set_aspect('equal', 'box')
    plt.tight_layout()
...
def visSimsVsDsts(sim, dst, uLab, rowLabels, colLabels, nSimBins=90, nDstBins=100, mxDst=None, clip=None, classLabels=None, fileName=None, tikzFileName=None):
    ...
    if mxDst is None:
        ...
        # mxDst =
        # numpy.sort(dst.flatten())[numpy.floor(dst.size*0.95).astype(numpy.int)]
        # # take the dst at 97%, the remaining 3% will be clipped to the top
        ...
        for lab in uLab:
            ...
            h, dstEdges, simEdges = numpy.histogram2d(
                ldsts, lsims, [nDstBins, nSimBins], range=[[mnDst, mxDst], [0., 180.]])
    ...
    for lab in uLab:
        ...
        if clip is not None:
            ...
            #   - by max
            h = h / numpy.tile(numpy.max(h, axis=1).reshape(nDstBins,
                                                            1) + 1, (1, nSimBins)).astype(numpy.float)
            # normalize columns
            #   - by max
...
#         h = h/numpy.tile(rads.reshape(1,nSimBins),(nDstBins,1))
    ...
    for lab in uLab:
        ...
        if numpy.mod(lab, nCols) == 0:
            ...
            # numpy.round(ticks*mxDst/float(nDstBins),1)
            labels = [''] * len(ticks)
...
def plotAngleErrors(sim, dst, maxAngle=40, maxK=25, kStep=3, fileName=None, tikzFileName=None, pklFileName=None, showPlot=True):
    ...
    * accuracy when matching to the best tmpl among the first k and allowing for angle error x
    ...
    def getGTAngleErrors(sim):
        ...
        perfAngleErr = numpy.arccos(
            numpy.maximum(sim[numpy.arange(sim.shape[0]), maxSimIdx], -1.)) * 180. / numpy.pi
        return perfAngleErr
    ...
    #             print minDstIdx0
        ...
        for i in xrange(k):
            angleErr[i, :] = numpy.arccos(numpy.maximum(
                sim[numpy.arange(nSamp), minDstIdx[:, i].flatten()], -1.)) * 180. / numpy.pi
        if k > 1:
    ...
    # accuracy when matching to the best tmpl among the first k and allowing
    # for angle error x
    ...
    plotAngleErrorsFig(lineData, lineLabels, colors, lineStyle, lineWidth, gtPerfIdx,
                       gtCumHst, maxAngle, fileName=fileName, tikzFileName=tikzFileName, showPlot=showPlot)

    if pklFileName is not None:
        with open(pklFileName, 'wb') as f:
            cPickle.dump({'lineData': lineData, 'lineLabels': lineLabels, 'colors': colors, 'lineStyle': lineStyle, 'lineWidth':
                          lineWidth, 'gtPerfIdx': gtPerfIdx, 'gtCumHst': gtCumHst, 'maxAngle': maxAngle}, f, protocol=cPickle.HIGHEST_PROTOCOL)
...
def plotAngleErrorsFig(lineData, lineLabels, colors, lineStyle, lineWidth, gtPerfIdx, gtLine, maxAngle, markers=None, title=None, fileName=None, tikzFileName=None, showPlot=True):
    ...
    # used to be "gt", now removed
    plt.plot([gtPerfIdx, gtPerfIdx], [0., 1.], 'r-.', label="")
    ...
    for i in xrange(len(lineData)):
        ...
        if markers is None:
            line, = plt.plot(
                ld, linestyle=lineStyle[i], label=lineLabels[i], linewidth=lineWidth[i])
        else:
            line, = plt.plot(ld, linestyle=lineStyle[i], marker=markers[
                             i], label=lineLabels[i], linewidth=lineWidth[i])
...
def visClosestTmpls(testdata_set, tmpl_set, dst, fileName=None, showPlot=True):
    ...
    if nChan == 1:
        ...
        dbgImg = numpy.zeros(
            (h * nSamp, w + b + w * nTmpl, nChan), dtype=img.dtype)
    else:
        dbgImg = numpy.zeros(
    ...
    # choose wisely
    validSampIdx, = numpy.in1d(
        testdata_set.y, numpy.array([0, 2, 3, 4, 5, 6, 8, 9, 11, 12, 14])).nonzero()
    ...
    if isinstance(testdata_set.x, list):
        ...
        testdata = numpy.concatenate(
            [x[validSampIdx[randIdx]] for x in testdata_set.x], axis=1)
    else:
        testdata = testdata_set.x[validSampIdx[randIdx]]

    if isinstance(tmpl_set.x, list):
        tmplData = numpy.concatenate(
            [x[tmpl_set.y >= 0] for x in tmpl_set.x], axis=1)
    else:
    ...
    for j in range(nSamp):
        ...
        for i in range(nTmpl):
            if nChan == 1:
                ...
                l = w + b + w * i
                tmplImg = numpy.rollaxis(
                    numpy.rollaxis(tmplData[minDstIdx[i]] + 0.5, 2), 2)
                dbgImg[h * j:h * (j + 1), l:l + w, :] = tmplImg
            else:
                l = 2 * w + b + 2 * w * i
                tmplImg = numpy.rollaxis(
                    numpy.rollaxis(tmplData[minDstIdx[i]], 2), 2)
...
def visWrongestClosestTmpls(testdata_set, tmpl_set, dst, sim, fileName=None, showPlot=True):
    ...
    if nChan == 1:
        dbgImg = numpy.zeros(
            ((h + border) * nRows, (2 * w + border) * nCols), dtype=img.dtype)
    else:
        dbgImg = numpy.zeros(
            ((h + border) * nRows, (2 * w + border) * nCols, 3), dtype=img.dtype)
    ...
    if isinstance(testdata_set.x, list):
        ...
        testdata = numpy.concatenate(
            [x[validSampIdx[minSimIdx]] for x in testdata_set.x], axis=ax)
    else:
    ...
    for j in range(nSamp):
        ...
        if nChan == 1:
            ...
            tmplImg = numpy.rollaxis(
                numpy.rollaxis(tmplData[tmplIdx] + 0.5, 2), 2)
            dbgImg[py:py + h, px + w:px + 2 * w, :] = tmplImg
        else:
